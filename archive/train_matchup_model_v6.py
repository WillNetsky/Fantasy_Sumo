# train_matchup_model_v6.py
"""
Matchup model v6 - Final push for 65%+ accuracy.
Key changes:
1. Feature interactions (rank x form, elo x age)
2. Polynomial features for key predictors
3. XGBoost with aggressive regularization
4. Hyperparameter optimization via Optuna-style grid
5. More historical context (career-level features)
"""
...
def compute_career_features(matches: pd.DataFrame) -> pd.DataFrame:
    ...
    results = []
    wrestler_ids = set(matches['winner_id'].tolist() + matches['loser_id'].tolist())
    ...
    for w_id in wrestler_ids:
        ...
        recent_10 = []  # last 10 match results

        prev_tournament = None
        for tourn in tournaments:
            # Stats BEFORE this tournament
            total = career_wins + career_losses
            career_win_pct = career_wins / max(1, total)

            # Recent form (last 10 matches weighted)
            if len(recent_10) > 0:
                weights = np.linspace(0.5, 1.0, len(recent_10))
                weighted_recent = np.average(recent_10, weights=weights)
            else:
                weighted_recent = 0.5
            ...
            results.append({
                ...
                'career_total_bouts': total,
                'weighted_recent_10': weighted_recent
            })
            ...
            recent_10.extend(t_matches['won'].tolist())
            recent_10 = recent_10[-10:]  # Keep last 10

            prev_tournament = tourn
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train matchup model v6.")
    ...
    parser.add_argument('--model-file', type=str, default="matchup_model_v6.joblib")
