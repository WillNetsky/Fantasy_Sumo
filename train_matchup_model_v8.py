# train_matchup_model_v8.py
"""
Matchup model v8 - Elo momentum features.
Key changes from v7:
1. Replace dict-based Elo lookup with compute_elo_features() from sumo_utils
2. Add Elo change windows: last 2/5/15 bouts and last 1/2 basho per wrestler
3. Diff features for each Elo change window (A minus B)
"""
...
def train_model(matchups: pd.DataFrame, model_output_path: str, run_ablation: bool = False, tune: bool = False):
    ...
    print("\n" + "=" * 50)
    print("NEW FEATURE IMPORTANCE (v8 elo momentum)", flush=True)
    print("=" * 50)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train matchup model v8.")
    ...
    parser.add_argument('--model-file', type=str, default="matchup_model_v8.joblib")
